...
class OptimReluMaxLinear(Function):
    @staticmethod
    def forward(ctx, x, weight, bias, mask, min_sparsity=0.01):
        '''
        ...
        n_nonzero = torch.count_nonzero(result)
        n_elem = result.numel()
        if (n_nonzero / n_elem) > min_sparsity:
            non_zero_elem = torch.nonzero(result, as_tuple=True)
            n_sample_elem = int(n_elem * min_sparsity)

            perm = torch.randperm(n_nonzero)
            sampled_indices = perm[:n_sample_elem]

            sample_mask = torch.zeros_like(result, dtype=torch.bool)
            sample_mask[non_zero_elem[0][sampled_indices], non_zero_elem[1][sampled_indices]] = True

            sampled_result = torch.where(sample_mask, result, 0.0)

            del perm, sampled_indices, sample_mask
            torch.cuda.empty_cache()

        else:
            sampled_result = result
 
        indice_not_zero = torch.nonzero(sampled_result, as_tuple=False)
        del sampled_result
        torch.cuda.empty_cache()
        ...
        del indice_not_zero, lmax, max_indice
        torch.cuda.empty_cache()
