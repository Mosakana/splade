...
from cuda_optimization import (
        SpladeTextEncoderV2Cuda, 
        CudaMaxAggregation)
from colbert import ColbertEncoder
from clusters import ClusterCompute, HFProjectionLayerLoader
...
def run(xp: IRExperimentHelper, cfg: SPLADE) -> PaperResults:
    ...
    laucher_clustering = find_launcher(cfg.clustering.requirements)
...
##########################################################################################
    ...
    colbert_tokenizer = HFStringTokenizer.from_pretrained_id(hf_id=model_id)
    encoder = HFTokensEncoder.C(model=ColbertEncoder.from_pretrained_id(model_id=model_id))
    text_encoder = TokenizedTextEncoder.C(encoder=encoder, tokenizer=colbert_tokenizer)

    cluster_linear_model = ClusterCompute.C(encoder=text_encoder, 
                                              documents=documents, 
    ...
    if cfg.splade.model == "splade_max":
        ...
        output_embeddings_loader = HFProjectionLayerLoader(linear=cluster_linear_model, model=encoder)

        splade_encoder = SpladeTextEncoderV2Cuda.C(
            tokenizer=HFTokenizerAdapter.C(
                tokenizer=tokenizer, converter=TopicTextConverter.C()
            ),
            encoder=encoder,
            aggregation=CudaMaxAggregation.C(),
            maxlen=256,
...
############################################################################################
    ...
    outputs = learner.add_pretasks(output_embeddings_loader).submit(launcher=gpu_launcher_learner)
